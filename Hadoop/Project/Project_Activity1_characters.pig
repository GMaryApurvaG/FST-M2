...
inputFile1 = LOAD 'hdfs:///user/apurva/episodeIV_dialogues.txt' using PigStorage('\t') AS (name,line);
inputFile2 = LOAD 'hdfs:///user/apurva/episodeV_dialogues.txt' using PigStorage('\t') AS (name,line);
inputFile3 = LOAD 'hdfs:///user/apurva/episodeVI_dialogues.txt' using PigStorage('\t') AS (name,line);
...
STORE Sorted_Output INTO 'hdfs:///user/apurva/Output_Characters';
